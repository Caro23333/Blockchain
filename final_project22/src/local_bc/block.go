...
// 12.6 添加
// 验证每一条交易，并给出不合法的交易
func (block *Block) ValidateTransaction(UTXO_set map[string]TransactionOutput, stale_message_set map[string]string, creation_reward float32) (bool, []Transaction, []Transaction) {
    var invalid_txs []Transaction
	var valid_txs []Transaction
	var used_UTXO_set map[string]bool = make(map[string]bool)
    res_flag := true
	has_creation_before := false
	last_tx := Transaction{} // 上一条交易，用来验证fee类型交易的母交易
	for i, tx := range block.Transactions {
		if !tx.Validate(used_UTXO_set, UTXO_set, stale_message_set, block.CreatorPublicKey, creation_reward, &has_creation_before, i, last_tx) {
			fmt.Println("Block invalid")
			res_flag = false
            invalid_txs = append(invalid_txs, tx)
			last_tx = Transaction{} // 避免利用无效交易或者过去的交易创建假的fee
		} else {
			valid_txs = append(valid_txs, tx)
			last_tx = tx
		}
	}
    return res_flag, invalid_txs, valid_txs
}
...
func (block *Block) Validate(UTXO_set map[string]TransactionOutput, stale_message_set map[string]string, creation_reward float32) bool {
    ...
    res_flag, _, _ := block.ValidateTransaction(UTXO_set, stale_message_set, creation_reward) // 12.6 修改 增加，实际不使用本函数
	return res_flag
}
