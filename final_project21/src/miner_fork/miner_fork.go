...
var node_list []network.NodeArgs = []network.NodeArgs{
	...
	{IP: "10.1.0.94", Port: "8058", PublicKey: *new(signature.JSONPublicKey), KeyValid: false},
}
...
var block_stash chan local_bc.Block // 挖矿成功后的区块暂存
...
func onNewBlockAppearance(new_block local_bc.Block) {
	...
	diff_mutex.Lock()
	if log_difficulty := new_block.LogDifficulty; !local_bc.ValidatePoW(log_difficulty, hash_bytes) || 
		new_block.Difficulty != 1<<log_difficulty || log_difficulty < overall_log_difficulty {
	...
	diff_mutex.Unlock()
	tx_mutex.Lock()
	for key := range transaction_pool {
		fmt.Println("Transaction in pool before validation", []byte(key))
	}
	...
	if res_flag, invalid_txs := new_block.ValidateTransaction(tmp_UTXO_set, stale_message_set, creation_reward); !res_flag {
		for _, tx := range invalid_txs {
			fmt.Println("Invalid transaction", local_bc.Hash(tx))
		...
		tx_mutex.Unlock()
		fmt.Println("--------------------Corrupted New Block Info ----------------------")
...
func onNewTransactionAppearance(new_tx local_bc.Transaction) {
	...
	if new_tx.Inputs == nil || new_tx.Outputs == nil || len(new_tx.Inputs) == 0 || len(new_tx.Outputs) == 0 {
		valid_flag = false
	} else {
...
// 该后台线程执行工作量证明，挖掘新的区块
func miningThread() {
	block_stash = make(chan local_bc.Block, 10)
	last_time_head := -1
	...
	for {
		...
		if local_bc.ValidatePoW(overall_log_difficulty, hash_bytes) {
			...
			for key, tx := range transaction_pool {
				...
				fmt.Println("Transaction in pool before broadcast", []byte(key))
				if flag, ok := transaction_flag[key]; !(ok && flag == 1) {
			...
			this_block := *try_next_block
			block_stash <- this_block // 暂存后再广播给其矿工
...
func BroadcastBlockThread() {
	time.Sleep(35 * time.Second)
	for {
		this_block := <-block_stash // 从暂存中取出
		network.BroadcastBlock(this_block) // 然后再广播给其矿工
...
func main() {
	...
	log_difficulty_ptr := flag.Int("log-difficulty", 21, "The number of prefix zeroes of SHA-256 required for PoW success")
	flag.Parse()
	my_ip = *my_ip_ptr
...
// 基于已确认的交易，客户端查询当前某一矿工的余额
func (t *ClientService) ClientQueryOneMiner(request_args *ClientQueryOneMinerArgs, reply *float32) error {
	*reply = QueryBalanceOneMiner(request_args.Account)
	return nil
}
...
// 查询某一矿工的余额
func QueryBalanceOneMiner(account int) float32 {
	if account >= len(node_list) {
		return -1
	}
	query_miner_public_key := node_list[account].PublicKey
	var sum float32 = 0
	chain_mutex.Lock()
	tx_mutex.Lock()
	for _, output := range main_UTXO_set {
		if PublicKeyEqual(output.PublicKey, query_miner_public_key) {
			sum += output.Value
		}
	}
	tx_mutex.Unlock()
