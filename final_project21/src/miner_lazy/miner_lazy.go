...
func onNewBlockAppearance(new_block local_bc.Block) {
	...
	fmt.Println("PoW value", hash_bytes)
	if !local_bc.ValidatePoW(new_block.LogDifficulty - 3, hash_bytes) {
		return // PoW 无效
	}
	tx_mutex.Lock()
...
func miningThread() {
	...
	// hash := sha256.New()
	for {
		// method := rand.Int() % 5 // 选择偷懒方式：0为瞎出块，1为降难度, 2为虚报难度
		chain_mutex.Lock()
		...
		/*if method == 0 {
			...
			*try_next_block = local_bc.Prepare(&block_pool[main_chain_head],
            ...
            overall_log_difficulty_cur := int(overall_log_difficulty / 2)
            if local_bc.ValidatePoW(overall_log_difficulty_cur, hash_bytes) {
				fmt.Println("!!!!!!! PoW succeed !!!!!!!")
				try_next_block.LogDifficulty = overall_log_difficulty_cur
				try_next_block.Difficulty = overall_difficulty
				diff_mutex.Unlock()
				tx_mutex.Lock()
				// 遍历交易池
		...
		if local_bc.ValidatePoW(overall_log_difficulty - 3, hash_bytes) {
			fmt.Println("!!!!!!! PoW succeed !!!!!!!")
			try_next_block.LogDifficulty = overall_log_difficulty
			try_next_block.Difficulty = overall_difficulty
			diff_mutex.Unlock()
