...
func onNewBlockAppearance(new_block local_bc.Block) {
	...
	tx_mutex.Lock()
	res_flag, _ := new_block.ValidateTransaction(tmp_UTXO_set, stale_message_set, creation_reward)//; !res_flag {
...
func (t *ClientService) ClientTransaction(request_args *ClientTransactionArgs, reply *float32) error {
	...
	*reply = collected_value // 如果一切正常，返回值为消耗的 UTXO 的总价值
	...
	inputs_another := make([]local_bc.TransactionInput, 0)
	tx_mutex.Lock()
	for _, output := range output_list {
		...
		message_another := local_bc.GetCurrentTime()
		r, s, _ = signature.SignMessage(my_private_key, message_another)
		inputs_another = append(inputs_another, local_bc.TransactionInput{
			M:             message_another,
			R:             r,
			S:             s,
			TransactionID: output.TransactionID,
	...
	tx_mutex.Unlock()
		...
		// 准备交易的 output, 再发一次给自己（malicious)
		outputs_another := make([]local_bc.TransactionOutput, 0)
		timestamp = local_bc.GetCurrentTime()
		var message_another bytes.Buffer
		message_another.Write(local_bc.Hash(my_public_key))
		binary.Write(&message_another, binary.LittleEndian, collected_value)
		message_another.Write(timestamp)
		r, s, _ = signature.SignMessage(my_private_key, message_another.Bytes())
		outputs_another = append(outputs_another, local_bc.TransactionOutput{
			PublicKey: my_public_key,
	...
	new_tx_another := local_bc.Transaction{
		Inputs:  inputs_another,
		Outputs: outputs_another,
	}
	onNewTransactionAppearance(new_tx)   // 先放进自己的交易池
	onNewTransactionAppearance(new_tx_another)   // 先放进自己的交易池
	network.BroadcastTransaction(new_tx) // 再广播给其他矿工
	network.BroadcastTransaction(new_tx_another) // 再广播给其他矿工另一个交易
	return nil
}
