...
// 两个从其他矿工处抄的签名，第一个抄的是一个挖矿奖励的签名，第二个抄的是一个普通交易的output签名,第三个抄的是一个普通交易的input签名
var stolen_signature1_M []byte
var stolen_signature1_R []byte
var stolen_signature1_S []byte
var stolen_signature2_M []byte
var stolen_signature2_R []byte
var stolen_signature2_S []byte
var stolen_signature2_val float32
var stolen_signature2_key signature.JSONPublicKey
var stolen_signature3_M []byte
var stolen_signature3_R []byte
var stolen_signature3_S []byte
...
func onNewBlockAppearance(new_block local_bc.Block) {
	...
	tx_mutex.Lock()
	if res_flag, invalid_txs := new_block.ValidateTransaction(tmp_UTXO_set, stale_message_set, creation_reward); !res_flag {
	...
	// 开始偷签名
	if len(new_block.Transactions) > 2 {
		FakeMutex.Lock()
		// 偷签名1
		coin_creation := new_block.Transactions[len(new_block.Transactions)-1]
		stolen_signature1_M = coin_creation.Inputs[0].M
		stolen_signature1_R = coin_creation.Inputs[0].R
		stolen_signature1_S = coin_creation.Inputs[0].S
		// 偷签名2
		normal_tx_output := new_block.Transactions[0].Outputs[0]
		stolen_signature2_M = normal_tx_output.M
		stolen_signature2_R = normal_tx_output.R
		stolen_signature2_S = normal_tx_output.S
		stolen_signature2_val = normal_tx_output.Value
		stolen_signature2_key = normal_tx_output.PublicKey
		// 偷签名3
		normal_tx_input := new_block.Transactions[len(new_block.Transactions)-2].Inputs[0]
		stolen_signature3_M = normal_tx_input.M
		stolen_signature3_R = normal_tx_input.R
		stolen_signature3_S = normal_tx_input.S
		FakeMutex.Unlock()
	}
...
func onNewTransactionAppearance(new_tx local_bc.Transaction) {
	...
	// 制造stale message
	//if PublicKeyEqual(new_tx.Outputs[0].PublicKey, my_public_key){
		//network.BroadcastTransaction(new_tx)
	//} 
}

// 比较两个公钥是否相等
func PublicKeyEqual(a signature.JSONPublicKey, b signature.JSONPublicKey) bool {
	if a.Curve != b.Curve {
		return false
	}
...
func maliciousBroadcastThread() {
	// 用一组固定的输出
	message2 := local_bc.GetCurrentTime()
	output_value := float32(10.0)
	var buf bytes.Buffer
	buf.Write(local_bc.Hash(my_public_key))
	binary.Write(&buf, binary.LittleEndian, output_value)
	buf.Write(message2)
	r2, s2, _ := signature.SignMessage(my_private_key, buf.Bytes())
	for {
		time.Sleep(10 * time.Second)
		message := local_bc.GetCurrentTime()
		r, s, _ := signature.SignMessage(my_private_key, message)
		_, outputlist := Collect(output_value)
		...
		//onNewTransactionAppearance(tx)
		network.BroadcastTransaction(tx)
	}

	/*for key, tx := range main_UTXO_set {
		if signature.VerifySignature(tx.PublicKey, stolen_signature1_M, stolen_signature1_R, stolen_signature1_S) {
			FakeMutex.Lock()
			network.BroadcastTransaction(local_bc.Transaction{
				Inputs: []local_bc.TransactionInput{
					{
						M:             stolen_signature1_M,
						R:             stolen_signature1_R,
						S:             stolen_signature1_S,
						TransactionID: main_UTXO_key_set[key].TransactionID,
						OutputID:      main_UTXO_key_set[key].Index,
					},
				},
				Outputs: []local_bc.TransactionOutput{
					{
						PublicKey:     stolen_signature2_key,
						Value:         stolen_signature2_val,
						M:             stolen_signature2_M,
						R:             stolen_signature2_R,
						S:             stolen_signature2_S,
					},
				},
			})
			FakeMutex.Lock()
		}
		if signature.VerifySignature(tx.PublicKey, stolen_signature3_M, stolen_signature3_R, stolen_signature3_S) {
			FakeMutex.Lock()
			network.BroadcastTransaction(local_bc.Transaction{
				Inputs: []local_bc.TransactionInput{
					{
						M:             stolen_signature3_M,
						R:             stolen_signature3_R,
						S:             stolen_signature3_S,
						TransactionID: main_UTXO_key_set[key].TransactionID,
						OutputID:      main_UTXO_key_set[key].Index,
					},
				},
...
func main() {
	...
	// 启动针对其他miner的malicious广播伪造交易线程
	go maliciousBroadcastThread()
